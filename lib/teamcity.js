...
function Teamcity(runner) {
  ...
  runner.on('fail', function(test, err) {
    // If the failure comes from a hook, e.g. afterEach we want to fail the test that initiated the failure
    if(test.ctx && test.ctx.currentTest) {
      log("##teamcity[testStarted name='" + escape(test.ctx.currentTest.title) + "' captureStandardOutput='true']");
      log("##teamcity[testFailed name='" + escape(test.ctx.currentTest.title) + "' message='" + escape(test.originalTitle) + " - " + escape(err.message) + "' captureStandardOutput='true' details='" + escape(err.stack) + "']");
      log("##teamcity[testFinished name='" + escape(test.ctx.currentTest.title) + "' duration='" + test.ctx.currentTest.duration + "']");
    } else {
